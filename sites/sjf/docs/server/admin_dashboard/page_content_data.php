...
if (!loggedIn()) {
  header('location: /server/logout.php?redirect=admin');
  exit(1);
}

if (!hasPermission('pageContentEditor', $_SESSION["user_role"])) {
  echo json_encode(array(
    'success' => false,
    'error_message' => 'You don\'t have permission to modify page content',
  ));
  exit(1);
}
...
forceFilePutContents($_SERVER['DOCUMENT_ROOT'] . '/data/page_content/page_content.json', $received_data);

$data_control = file_get_contents($_SERVER['DOCUMENT_ROOT'] . '/data/data_control.json');
$json_data = json_decode($data_control, true);
$json_data['page_content/page_content.json'] = time();
