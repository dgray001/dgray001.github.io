...
/**
 * Assumed sjf_users table schema (inferred from CUF's cuf_users; not yet
 * reviewed against a real SJF database):
 *   id, email, password_hash, role, activated,
 *   activation_code, activation_code_expiration,
 *   last_logged_in, last_activated
 */
...
function userEmailExists($conn, $email): array {
  $sqlCommand = "SELECT * FROM sjf_users u WHERE u.email = ?;";
...
function loginUser($conn, $email, $password): string {
  ...
  $time = time();
  $userid = $user["id"];
  $cmd = "UPDATE sjf_users u SET last_logged_in = ? WHERE u.id = ?;";
  $stmt = mysqli_stmt_init($conn);
...
function sendVerificationCode($conn, $email, $expect_activated, $expect_logged_in): string {
  ...
  $cmd = 'UPDATE sjf_users SET activation_code = ?, activation_code_expiration = ? WHERE email = ?';
  $stmt = mysqli_stmt_init($conn);
  ...
  <html>
    <head>
      <title>SJF Single Use Email Verification Code</title>
    </head>
    <body>
      <p>
        Below is your single-use email verification code. This code will expire in 10 minutes. If you did not request this code, someone may be trying to access your St. Joseph Foundation account
      </p>
      <p>
        $code
      </p>
  ...
  ";
  if (!sendEmail($email, 'SJF Single Use Email Verification Code', json_decode(json_encode($email_body)))) {
