...
if (!hasPermission('chapters', $_SESSION["user_role"])) {
  echo json_encode(array(
    'success' => false,
    'error_message' => 'You don\'t have permission to modify chapters data',
  ));
  exit(1);
}
...
forceFilePutContents($_SERVER['DOCUMENT_ROOT'] . '/data/chapters/chapters.json', $received_data);

$data_control = file_get_contents($_SERVER['DOCUMENT_ROOT'] . '/data/data_control.json');
$json_data = json_decode($data_control, true);
$json_data['chapters/chapters.json'] = time();
